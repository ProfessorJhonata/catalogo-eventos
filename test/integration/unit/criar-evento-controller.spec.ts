...
class CriarEventoController {
    async handle(req: Request, res: Response): Promise<Response> {
        ...
        if (!req.body?.titulo){
            return res.status(400).json({ error: 'Dados invalidos para a criação de evento'})
        }
...
describe('CriarEventoController', () => {
    ...
    it('shoud handle event creation', async () => {
    ...
    it('should fail to create event with invalid data', async() => {
        ...
        expect(res.json).toHaveBeenCalledWith({ error: 'Dados invalidos para a criação de evento'})
    })
